...
class Order(models.Model):  # Заказ
    ...
    products = models.ManyToManyField(CartItem)  # продукт в заказе
    ...
    def add_item(self, products, quantity):
        _price = CartItem.objects.item_price(products) * quantity
    # cart_price = product.price * quantity
        if not self.products.filter(product=products).exists():
            cart_item = CartItem.objects.create(
                order=self, products=products, quantity=quantity, cart_price=_price)
            self.products.add(cart_item)
        else:
            existing_item = self.products.get(products=products)
